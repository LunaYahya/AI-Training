...
with left:
    ...
    if uploaded is None:
        ...
        st.image(preview, caption=f'Uploaded image · {uploaded.name}', use_column_width=True)

with right:
    st.markdown('### 2 · Review the result')
    predict_clicked = st.button('Run prediction', type='primary', use_column_width=True)
